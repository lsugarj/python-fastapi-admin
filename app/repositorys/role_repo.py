...
from app.models.rbac import Role, RoleMenu
...
class RoleRepository:
    ...
    @staticmethod
    async def create_role(role: Role, menu_ids: list[int], session: AsyncSession) -> int:
        # 创建用户
        session.add(role)
    ...
    @staticmethod
    async def update_role(role_id: int, update_data: dict, menu_ids: list[int], session: AsyncSession) -> bool:
        """
        返回：
        True  → 更新成功
    ...
    async def get_role_page(params: RolePageQueryParams, session: AsyncSession):
        ...
        # ===== 分页 =====
        query_stmt = stmt.options(selectinload(Role.menus)).offset(params.offset).limit(params.size)
